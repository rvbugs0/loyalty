...
select `customer`.`code` AS `customer_code`, trim(`customer`.`name`) AS `name`,trim(`customer`.`username`) AS `username`,
trim(`customer`.`password`) AS `password`, trim(`customer`.`password_key`) AS `password_key`,
trim(`customer`.`permanent_address`) AS `permanent_address`,trim(`customer`.`current_address`) AS `current_address`,
`customer`.`city_code` AS `city_code`,trim(`city`.`name`) AS `city_name`, trim(`city`.`state`) AS `state`,
trim(`city`.`country`) AS `country`,`customer`.`date_of_birth` AS `date_of_birth`, trim(`customer`.`gender`) AS `gender`,
trim(`customer`.`occupation`) AS `occupation`,`customer`.`is_married` AS `is_married`,`customer`.`is_student` AS `is_student`,
trim(`customer`.`contact_number`) AS `contact_number`,trim(`customer`.`mail_id`) AS `mail_id` 
from (`customer` inner join `city` on((`customer`.`city_code` = `city`.`code`))); 
...
/*customer exist by contact number */
...
/*get all customers by occupation */
﻿DELIMITER $$
...
/*get count of married customers */
﻿DELIMITER $$
...
/* get count of student customers*/
...
/* get customer count by dob*/
﻿DELIMITER $$

DROP PROCEDURE IF EXISTS `loyalty`.`get_customer_count_by_date_of_birth` $$
CREATE PROCEDURE `loyalty`.`get_customer_count_by_date_of_birth` (in v_date_of_birth int,out v_count int)
BEGIN
declare customer_count int;
...
/*get customer count by occupation */
﻿DELIMITER $$

DROP PROCEDURE IF EXISTS `loyalty`.`get_customer_count_by_occupatiion` $$
CREATE PROCEDURE `loyalty`.`get_customer_count_by_occupatiion` (in v_occupation char(30),out v_count int)
BEGIN
declare customer_count int;
