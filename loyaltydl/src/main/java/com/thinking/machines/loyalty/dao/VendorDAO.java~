...
//not -tested
	public ArrayList<VendorInterface> getByCityCode(int cityCode,Connection connection) throws DAOException
	{
		boolean closeConnection=false;
		try
		{
			...
			String job="{ call get_vendors_by_city_code(?) }";
			...
			if(closeConnection)
{
	connection.close();
			...
			throw new DAOException("VendorDAO : getByCityCode() --> No ResultSet object");
			}
			ResultSet resultSet=callableStatement.getResultSet();
			if(resultSet.next()==false)
			{
				...
				if(closeConnection)
{
	connection.close();
				...
				throw new DAOException("VendorDAO : getByCityCode() --> No records ");
			}
		...
		{
			throw new DAOException("VendorDAO : getByCityCode() " + exception.getMessage());
		}
